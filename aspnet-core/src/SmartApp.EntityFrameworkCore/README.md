...
````bash
Add-Migration Your_Migration_Name -Context SmartAppDbContext
````

Using EF Core command line tool;

````bash
dotnet ef migrations add Your_Migration_Name --context SmartAppDbContext
````
...
````bash
Add-Migration Your_Migration_Name -Context SmartAppTenantDbContext -OutputDir TenantMigrations
````

Using EF Core command line tool:

````bash
dotnet ef migrations add Your_Migration_Name --context SmartAppTenantDbContext --output-dir TenantMigrations
````
